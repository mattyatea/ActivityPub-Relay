 ...
 * - 配信失敗は警告ログに記録されます
 */
export const relayActivity = async (activity: APActivity, context: Context<{
    Bindings: Bindings
}>): Promise<{ success: boolean; relayedCount: number; failureCount: number }> => {
    if (!checkPublicCollection(activity)) {
        return { success: false, relayedCount: 0, failureCount: 0 };
    }

    const {results: followers} = await context.env.DB.prepare("SELECT id, inbox, sharedInbox FROM actors").all<{
        id: string;
        inbox: string;
