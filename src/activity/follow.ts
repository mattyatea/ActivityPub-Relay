 ...
 */
export const followActivity = async (activity: APActivity, actor: APActor, context: Context<{
    Bindings: Bindings
}>): Promise<boolean> => {
    if (checkPublicCollection(activity)) {
        ...
        const {results: actorExists} = await context.env.DB.prepare("SELECT id FROM actors WHERE id = ?").bind(activity.actor).all();
        if (!actorExists || actorExists.length === 0) {
            await context.env.DB.prepare("INSERT INTO actors (id, inbox, sharedInbox, publicKey) VALUES (?, ?, ?, ?)")
            ...
            await context.env.DB.prepare("UPDATE actors SET inbox = ?, sharedInbox = ?, publicKey = ? WHERE id = ?")
        ...
        const {results: followRequestExists} = await context.env.DB.prepare("SELECT id FROM followRequest WHERE id = ?").bind(activity.id).all();
        if (!followRequestExists || followRequestExists.length === 0) {
            ...
            await context.env.DB.prepare("INSERT INTO followRequest (id, actor, object) VALUES (?, ?, ?)")
